...
const debug = debugFn(`capsule:tasks:start:system`)

export default class TasksStartSystemCommand extends Command {

  static description = `Start a system task with the given configuration`
  static hidden = true

  static flags = { // how to change ordering of these in help command?
  ...
  async run(): Promise<void> {
    const { flags } = await this.parse(TasksStartSystemCommand)
    ...
    try {
        const task: NewTask = await createTask(flags, `system`) as NewTask
